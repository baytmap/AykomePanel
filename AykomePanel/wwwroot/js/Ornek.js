/*import { statesData } from "../js/us-states.js";*/
const CizimTip = {
    Marker: "Marker",
    Circle: "Circle",
    Polygon: "Polygon",
...
document.onkeydown = function (evt) {
    ...
    if (isEscape) {
        if (drawCurpolygon != null) {
            map.removeLayer(drawCurpolygon);
            map.eachLayer(function (a) {
                if (a.options.ilkIslem)
                    map.removeLayer(a);
            });
            ozPolgArray = [];
            drawCurpolygon = null;
...
var ozPolgArray = [];
var cizimTipi = CizimTip.None;
var drawCurpolygon = null,
    poligonilkRect = null,
    vvIndex = 1;
...
function onMapClick(e) {
    ...
    ozPolgArray.push({ lat: e.latlng.lat, lng: e.latlng.lng });

    if (cizimTipi == CizimTip.Circle) {
    ...
    else if (cizimTipi == CizimTip.Marker) {
        L.marker([e.latlng.lat, e.latlng.lng]).addTo(map);
        ozPolgArray = [];
    }

    else if (cizimTipi == CizimTip.Polygon && ozPolgArray.length > 1) {

        if (drawCurpolygon != null)
            map.removeLayer(drawCurpolygon);

        if (ozPolgArray.length > 1) {
            ...
            var ss = zoomRectAyr.find(q => q.Zoom == map.getZoom()).Deger;
            for (var i = 0; i < ozPolgArray.length; i++) {
                var boundsrectangle = [
                    [(ozPolgArray[i].lat - ss), (ozPolgArray[i].lng - ss)],
                    [(ozPolgArray[i].lat + ss), (ozPolgArray[i].lng + ss)]
                ];

                var rectangle = L.rectangle(boundsrectangle, {
                    color: i > 0 ? "#b95e13;" : "#dc3545",
                    weight: 1,
                    ilkIslem: true
                }).addTo(map);

                if (i == 0) {
                    poligonilkRect = rectangle;
                    poligonilkRect.on("click", function (event) {
                        var krdAry = [];
                        for (var h = 0; h < ozPolgArray.length; h++) {
                            var virg = ozPolgArray.length == h + 1 ? '' : ',';
                            // krdAry += `[${ozPolgArray[h].lat}, ${ozPolgArray[h].lng}]${virg}`;
                            krdAry.push([ozPolgArray[h].lng, ozPolgArray[h].lat]);
                        }

                        ozPolgArray = [];
                        // var virg2 = jsoonData == '' ? '' : ',';
                        // jsoonData += '' + virg2 + '{"type": "Feature","id": "01","properties": { "name": "Alabama", "density": 94.65 },"geometry": { "type": "Polygon", "coordinates": [[' + krdAry + ']] } }';
                        jsoonData.push({ "type": "Feature", "id": "" + vvIndex + "", "properties": { "name": "Alabama", "density": 94.65 }, "geometry": { "type": "Polygon", "coordinates": [krdAry] } })
                        drawCurpolygon = null;
                        poligonilkRect = null;
                        vvIndex += 1;
                        map.eachLayer(function (a) {
                        ...
                        setTimeout(() => { ozPolgArray = []; }, 1000)
                    });
                }
            }
            drawCurpolygon = L.polygon(ozPolgArray, { color: "#ff7800", weight: 2 }).addTo(map);
        }
    }
    //katman = L.popup();
    //katman.setLatLng(e.latlng)
...
const fncAddJson = () => {
    ...
    var mstatesData = {
        "type": "FeatureCollection", "features": jsoonData
    };
    console.log(mstatesData);

    katman = L.geoJson(mstatesData, {
...
function fncKatmanKaldir() {
    ...
    //katman.removeFrom(map);

    //L.control.removeLayer(katman);
...
const fncAddMarker = () => {
    katman = L.marker([51.5, -0.09]).addTo(map);

    katman.bindPopup("<b>Hello world!</b><br>I am a popup.").openPopup();

}

const fncAddCircle = () => {
    katman = L.circle([51.508, -0.11], {
        color: 'red',
        fillColor: '#f03',
        fillOpacity: 0.5,
        radius: 500
    ...
    katman.bindPopup("I am a circle.");
}
...
const fncAddPolygon = () => {
    ...
    katman.bindPopup("I am a polygon.")
}
...
const fncAddJsonPoint = () => {
    ...
    //    }
    //};
    katman = L.geoJSON(jsoonData).addTo(map);

    // Ortalama
    map.fitBounds(katman.getBounds());
}

const fncAddJsonLine = () => {
    var myLines = [{
        "type": "LineString",
...
//* onEachFeature seçeneği, her özelliği bir GeoJSON katmanına eklemeden önce çağrılan bir işlevdir. Bu seçeneği kullanmanın yaygın bir nedeni, özelliklere tıklandığında bir açılır pencere eklemektir. */

const fncAddJsonPolygon2 = () => {
    var geojsonFeature = {
        "type": "Feature",
        "properties": {
            "name": "Coors Field",
    ...
    katman = L.geoJSON(geojsonFeature, {
...
//*** Alternatif olarak boş bir GeoJSON katmanı oluşturabilir ve onu bir değişkene atayarak daha sonra ona daha fazla özellik ekleyebiliriz.
...
// #region Sicaklik Haritası
var info;
const fncSicaklikHaritasi = () => {
...
function fncLejyanEkle() {
    ...
    legend.onAdd = function (map) {
        ...
        for (var i = 0; i < grades.length; i++) {
            div.innerHTML +=
                '<i style="background:' + getSicColor(grades[i] + 1) + '"></i> ' +
...
const fncAddMarkerKatmanOlustur = () => {
    ...
    katman = L.control.layers(baseMaps, overlayMaps).addTo(map);
}

const fncAddMarkerKatmanEkle = () => {
    ...
    katman.addBaseLayer(openTopoMap, "OpenTopoMap");
    katman.addOverlay(parks, "Parks Katamanı");
}
...
function fncZoomDurumlari() {
    ...
    //setView(center, zoom), bu aynı zamanda harita merkezini de ayarlar

    //flyTo(center, zoom), setView gibi ama düzgün bir animasyonla

    //zoomIn() / zoomIn(delta), delta yakınlaştırma düzeylerinde yakınlaştırır, varsayılan olarak 1

    //zoomOut() / zoomOut(delta), varsayılan olarak 1 olan delta yakınlaştırma düzeylerini uzaklaştırır

    //setZoomAround(fixedPoint, zoom), bir noktayı sabit tutarken yakınlaştırma düzeyini ayarlar(kaydırma tekerleği yakınlaştırmanın yaptığı şey)

    //fitBounds(bounds), haritada dikdörtgen bir alana sığacak şekilde yakınlaştırmayı otomatik olarak hesaplar
...
const fncAddWms = () => {
    ...
    // //katman = L.tileLayer.wms('http://ows.mundialis.de/services/service?', wmsOptions).addTo(map);

    // katman = L.tileLayer.wms('http://ows.mundialis.de/services/service?', {
    // layers: 'TOPO-OSM-WMS'
    // }).addTo(map);
    ...
    //Ayrıca Leaflet'in çok az sayıda koordinat sistemini desteklediğini
    //unutmayın: CRS: 3857, CRS: 3395 ve CRS: 4326(L.CRS belgelerine bakın).WMS hizmetiniz bu koordinat
    //sistemlerinde görüntüler sunmuyorsa Leaflet'te farklı bir koordinat sistemi kullanmak için
    //Proj4Leaflet'i kullanmanız gerekebilir.Bunun dışında, haritanızı başlatırken yalnızca doğru CRS'yi
    //kullanın; eklenen tüm WMS katmanları onu kullanacaktır.
...
const fncAddTms = () => {
    ...
    katman = L.tileLayer('http://base_url/tms/1.0.0/example_layer@png/{z}/{x}/{y}.png', {
        tms: true
    }).addTo(map);
}
